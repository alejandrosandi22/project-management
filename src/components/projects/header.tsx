export default function Header() {
  return (
    <>
      <header className='flex justify-between items-center'>
        <div className='font-semibold text-black dark:text-white'>Projects</div>
        <div className='space-x-5'>
          <button className='bg-primary-700 text-white dark:bg-primary-500 hover:bg-primary-900 hover:dark:bg-primary-700 rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center'>
            <svg
              xmlns='http://www.w3.org/2000/svg'
              viewBox='0 0 24 24'
              className='w-5 h-5 mr-2 -ml-1 text-white'
            >
              <path fill='none' d='M0 0h24v24H0z' />
              <path
                d='M6.17 18a3.001 3.001 0 0 1 5.66 0H22v2H11.83a3.001 3.001 0 0 1-5.66 0H2v-2h4.17zm6-7a3.001 3.001 0 0 1 5.66 0H22v2h-4.17a3.001 3.001 0 0 1-5.66 0H2v-2h10.17zm-6-7a3.001 3.001 0 0 1 5.66 0H22v2H11.83a3.001 3.001 0 0 1-5.66 0H2V4h4.17zM9 6a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm6 7a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm-6 7a1 1 0 1 0 0-2 1 1 0 0 0 0 2z'
                fill='currentColor'
              />
            </svg>
            Add filter
          </button>
          <button className='border border-primary-100 dark:border-primary-50 hover:bg-primary-700/5 hover:dark:bg-primary-500/20 rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center'>
            <svg
              xmlns='http://www.w3.org/2000/svg'
              viewBox='0 0 24 24'
              className='w-5 h-5 mr-2 -ml-1 text-black dark:text-white'
            >
              <path fill='none' d='M0 0h24v24H0z' />
              <path
                d='M4 3h16a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm1 2v14h14V5H5zm6 6V7h2v4h4v2h-4v4h-2v-4H7v-2h4z'
                fill='currentColor'
              />
            </svg>
            New project
          </button>
        </div>
      </header>
    </>
  );
}
